...
{{-- <div x-data={playing:false,muted:false} @click.outside="$refs.player.pause()" x-intersect:leave="$refs.player.pause()">
    <video x-ref="player" @play="playing=true" @pause="playing=false" controls class="w-full h-[500px] rounded-md">
...
}" x-init="supportsVideo = document.createElement('video').canPlayType;
if (!supportsVideo) {
...
document.addEventListener('fullscreenchange', (e) => {
    ...
    <div x-show="videoPlayerReady" class="absolute inset-0 w-full h-full">
        <div x-ref="videoBackground" @click="togglePlay()"
            class="absolute inset-0 z-30 flex items-center justify-center w-full h-full bg-black bg-opacity-0">
            <div x-show="playing" x-transition:enter="transition ease-out duration-1000"
                x-transition:enter-start="scale-50 opacity-100" x-transition:enter-end="scale-100 opacity-0"
                class="flex items-center justify-center w-20 h-20 bg-black rounded-full opacity-0 bg-opacity-20"
                x-cloak>
                <svg class="w-10 h-10 translate-x-0.5 text-white" viewBox="0 0 24 24" fill="none"
            ...
            <div x-show="!playing && !ended" x-transition:enter="transition ease-out duration-1000"
                x-transition:enter-start="scale-50 opacity-100" x-transition:enter-end="scale-100 opacity-0"
                class="flex items-center justify-center w-20 h-20 bg-black rounded-full opacity-0 bg-opacity-20"
                x-cloak>
        ...
        <div x-show="controls" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="translate-y-full" x-transition:enter-end="translate-y-0"
            x-transition:leave="transition ease-in duration-300" x-transition:leave-start="translate-y-0"
            x-transition:leave-end="translate-y-full"
            class="absolute bottom-0 left-0 z-20 w-full h-1/2 opacity-20 bg-gradient-to-b from-transparent to-black"
            x-cloak>
        </div>
        <div x-show="controls" @click="resetControlsTimeout" x-transition:enter="transition ease-out duration-300"
            x-transition:enter-start="translate-y-full" x-transition:enter-end="translate-y-0"
            x-transition:leave="transition ease-in duration-300" x-transition:leave-start="translate-y-0"
            x-transition:leave-end="translate-y-full" class="absolute bottom-0 left-0 z-40 w-full h-12" x-cloak>
            <ul class="absolute bottom-0 left-0 z-20 flex items-center w-full text-white">
                <li class="inline">
                    <button @click="togglePlay()"
                        class="flex items-center justify-center w-10 h-10 duration-150 ease-out opacity-80 hover:opacity-100"
                        type="button">
                        <svg x-show="!playing" class="w-5 h-5" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M8.42737 3.41611C6.46665 2.24586 4.00008 3.67188 4.00007 5.9427L4 18.0572C3.99999 20.329 6.46837 21.7549 8.42907 20.5828L18.5698 14.5207C20.4775 13.3802 20.4766 10.6076 18.568 9.46853L8.42737 3.41611Z"
                ...
                <li class="w-full">
                    <div class="relative w-full h-2 rounded-full">
                        <input x-ref="videoProgress" @click="timelineClicked" @input="timelineSeek(event)"
                ...
                <li class="flex items-center group">
                    <button @click="toggleMute()" type="button"
                        class="flex items-center justify-center w-6 h-10 duration-150 ease-out opacity-80 hover:opacity-100">
                        <svg x-show="!muted" class="w-[18px] h-[18px]" xmlns="http://www.w3.org/2000/svg"
                    ...
                    <div
                        ...
                        <input x-ref="volume" @input="updateVolume(event)" type="range" min="0"
                            max="1" :value="volume" step="0.01"
                            class="w-full h-full appearance-none flex items-center cursor-pointer bg-transparent z-30
                ...
                <li class="ml-auto">
                    <button x-ref="fullscreenButton" @click="handleFullscreen"
